...
Public Class dBTools
    ...
    Private mCNN As SqlConnection = Nothing
    Private mTran As SqlTransaction = Nothing
    Private m_CommandTimeout As Integer = -1

    ' ***************************************************************************************************
    ' * Inicializa la Clase
    ' ***************************************************************************************************
    Public Sub New()
        Me.New(ConfigurationManager.ConnectionStrings("dbCNN2").ConnectionString, -1)
        'Me.New(clsRegistry.CNN, -1)
    End Sub

    Public Sub New(ByVal strCNX As String)
        Me.New(strCNX, -1)
    End Sub
    ...
    Public Sub New(ByVal CommandTimeout As Integer)
        Me.New(ConfigurationManager.ConnectionStrings("dbCNN2").ConnectionString, CommandTimeout)
        'Me.New(clsRegistry.CNN, CommandTimeout)
    End Sub

    Public Sub New(ByVal strCNX As String, ByVal CommandTimeout As Integer)
        Me.New(New SqlConnection(strCNX), CommandTimeout)
    End Sub

    Public Sub New(ByVal CNX As SqlConnection, ByVal CommandTimeout As Integer)
        mCNN = CNX
    ...
    Public Sub RollBack()
        If mTran IsNot Nothing Then
            mTran.Rollback()
        End If
    End Sub

    ' ***************************************************************************************************
    ' * Funciones con Regreso de Datos SQLDataReader y DataSet
    ' ***************************************************************************************************
    Public Function spExec(ByVal spName As String, ByVal ParamArray Parametros() As SqlParameter) As SqlDataReader
        Try
            If mTran Is Nothing Then
                Return SqlHelper.ExecuteReader(mCNN, CommandType.StoredProcedure, spName, Parametros)
            Else
                Return SqlHelper.ExecuteReader(mTran, CommandType.StoredProcedure, spName, Parametros)
            End If
    ...
    Public Function sqlQuery(ByVal Query As String, ByVal ParamArray Parametros() As SqlParameter) As SqlDataReader
        Try
            If mTran Is Nothing Then
                Return SqlHelper.ExecuteReader(mCNN, CommandType.Text, Query, Parametros)
            Else
                Return SqlHelper.ExecuteReader(mTran, CommandType.Text, Query, Parametros)
            End If
        Catch ex As Exception
            Throw ex
            Return Nothing
    ...
    Public Function spExecDataSet(ByVal spName As String, ByVal ParamArray Parametros() As SqlParameter) As DataSet
        Try
            If mTran Is Nothing Then
                Return SqlHelper.ExecuteDataset(mCNN, CommandType.StoredProcedure, spName, Parametros)
            Else
                Return SqlHelper.ExecuteDataset(mTran, CommandType.StoredProcedure, spName, Parametros)
            End If
        Catch ex As Exception
            Throw ex
            Return Nothing
    ...
    Public Function sqlQueryDataSet(ByVal Query As String, ByVal ParamArray Parametros() As SqlParameter) As DataSet
        Try
            If mTran Is Nothing Then
                Return SqlHelper.ExecuteDataset(mCNN, CommandType.Text, Query, Parametros)
            Else
                Return SqlHelper.ExecuteDataset(mTran, CommandType.Text, Query, Parametros)
            End If
        Catch ex As Exception
            Throw ex
            Return Nothing
    ...
    ' ***************************************************************************************************
    ' * Funciones sin Regreso
    ' ***************************************************************************************************
    Public Function spExecNonQuery(ByVal spName As String, ByVal ParamArray Parametros() As SqlParameter) As Integer
        Try
            If mTran Is Nothing Then
                Return SqlHelper.ExecuteNonQuery(mCNN, CommandType.StoredProcedure, spName, Parametros)
            Else
                Return SqlHelper.ExecuteNonQuery(mTran, CommandType.StoredProcedure, spName, Parametros)
            End If
        Catch ex As Exception
            Throw ex
            Return Nothing
    ...
    Public Function sqlQueryNonQuery(ByVal Query As String, ByVal ParamArray Parametros() As SqlParameter) As Integer
        Try
            If mTran Is Nothing Then
                Return SqlHelper.ExecuteNonQuery(mCNN, CommandType.Text, Query, Parametros)
            Else
                Return SqlHelper.ExecuteNonQuery(mTran, CommandType.Text, Query, Parametros)
            End If
        Catch ex As Exception
            Throw ex
            Return Nothing
    ...
    ' ***************************************************************************************************
    ' * Funciones Regresa el valor de la Primera Fila Primera Columna
    ' ***************************************************************************************************
    Public Function spExecScalar(ByVal spName As String, ByVal ParamArray Parametros() As SqlParameter) As Object
        Try
            If mTran Is Nothing Then
                Return SqlHelper.ExecuteScalar(mCNN, CommandType.StoredProcedure, spName, Parametros)
            Else
                Return SqlHelper.ExecuteScalar(mTran, CommandType.StoredProcedure, spName, Parametros)
            End If
        Catch ex As Exception
            Throw ex
            Return Nothing
    ...
    Public Function sqlQueryScalar(ByVal Query As String, ByVal ParamArray Parametros() As SqlParameter) As Object
        Try
            If mTran Is Nothing Then
                Return SqlHelper.ExecuteScalar(mCNN, CommandType.Text, Query, Parametros)
            Else
                Return SqlHelper.ExecuteScalar(mTran, CommandType.Text, Query, Parametros)
            End If
        Catch ex As Exception
            Throw ex
            Return Nothing
    ...
    ' ***************************************************************************************************
    ' * Funcion Que convierte de un SQLDataReader a DataSet
    ' ***************************************************************************************************
    Public Function ConvertDataReaderToDataSet(ByVal reader As SqlDataReader) As Object
    ...
    Public Function cArrayList(ByVal Col As SqlDataReader) As ArrayList
        Dim Result As New ArrayList

        Try
            Dim CellName(Col.FieldCount - 1) As Object
            For iCol As Integer = 0 To Col.FieldCount - 1
                CellName(iCol) = Col.GetName(iCol)
            Next
            Result.Add(CellName)

            While (Col.Read)
                Dim CellData(Col.FieldCount - 1) As Object
                For iCol As Integer = 0 To Col.FieldCount - 1
                    CellData(iCol) = Col.Item(iCol)
                Next
                Result.Add(CellData)
            End While
        ...
        Catch ex As OleDbException
            Throw ex
        Finally
            If Col IsNot Nothing AndAlso Not Col.IsClosed Then
                Col.Close()
            End If
        End Try
    ...
    Public Function spExist(ByVal spName As String) As Boolean
        Dim Query As String = "SELECT Count(*) as Cant FROM INFORMATION_SCHEMA.ROUTINES WHERE Routine_Type = N'Procedure' and SPECIFIC_NAME = @spName"
        Dim Col As Object = Nothing
        Dim Result As Boolean = False

        Try
            Col = sqlQueryScalar(Query, New SqlParameter("@spName", spName))

            If Col IsNot Nothing AndAlso IsNumeric(Col) AndAlso Col > 0 Then
                Result = True
            Else
                Result = False
            End If
    ...
    Public Function tbExist(ByVal tbName As String) As Boolean
        Dim Query As String = "SELECT COUNT(1) As Cant FROM sysobjects WHERE xType = N'U' and Name = @tbName"
        Dim Col As Object = Nothing
        Dim Result As Boolean = False

        Try
            Col = sqlQueryScalar(Query, New SqlParameter("@tbName", tbName))
    ...
    Public Function dbExists(ByVal DataBaseName As String) As Boolean
        Dim Query As String = "SELECT CASE WHEN Exists(SELECT 1 FROM Master.dbo.SysDataBases WHERE name = @DB) THEN CAST(1 As bit) ELSE CAST(0 As bit) END dbExists"
        Dim Result As Boolean = False

        Try
            Result = CBool(sqlQueryScalar(Query, New SqlParameter("@DB", DataBaseName)))
        Catch ex As Exception
            Result = False
        End Try
    ...
    Public ReadOnly Property strCNN() As String
        Get
            Return mCNN.ConnectionString
        End Get
    End Property

    Public Overloads Function toString(ByVal spName As String, ByVal ParamArray Parametros() As SqlParameter) As String
        Dim Paso As String = "Exec " & spName & " "

        For Each P As SqlParameter In Parametros
            Paso &= P.ParameterName.ToString
            Paso &= " = "
            If P.SqlDbType = Data.SqlDbType.NVarChar OrElse P.SqlDbType = Data.SqlDbType.VarChar OrElse P.SqlDbType = Data.SqlDbType.DateTime OrElse P.SqlDbType = Data.SqlDbType.Date Then
                Paso &= "'"
            End If
            Paso &= P.SqlValue.ToString
            If P.SqlDbType = Data.SqlDbType.NVarChar OrElse P.SqlDbType = Data.SqlDbType.VarChar OrElse P.SqlDbType = Data.SqlDbType.DateTime OrElse P.SqlDbType = Data.SqlDbType.Date Then
                Paso &= "'"
            End If
        ...
        Return Paso.Trim.Substring(0, Paso.Trim.Length - 1)
    End Function
...
Public NotInheritable Class SqlHelper
    Private Shared m_CommandTimeout As Integer = -0
