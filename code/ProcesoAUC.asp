...
If ISNUMERIC(iCodigoAcceso) then
			...
			sSQL="EXEC spInsDatosActivacion " & iCodigoAcceso & ",'"& sNombreAsegurado &"','"& sTelefono &"','"& sEmail &"',"& iAnio & "," & iMarca & ",'"& sModelo &"','"& sSerie &"','"& sPlacas &"','"& sFechaInicio &"','"& sHoraInicio & "','Web'"
			'response.write sSQL
			...
			'5.- COnsultamos el numero de poliza de Web
				...
				'3.-Insertamos en Poliza de Autos
						...
						sSQLInsPoliza_de_autosUC = "sp_InsPoliza_de_Autos 'AXW'," & Folio & ",'TX',1,'"& sFechaInicio &"','"& sfecha &"','"& sHoraInicio &"','" & _
						sNombreAsegurado & "','N/A','NA','0','+52'," &  formateaString(sTelefono) & "," & formateaString(sProducto) & ",'NL','NA','NA',127,1," & _
				...
				response.redirect("resultadoActivacion.asp?m=1&co=" &  sPolizaUCWeb & "&f=" & Folio)
				response.end()
				'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
Else
	m=4
